...
def get_wiki_statistics(wiki_name, include_zero=True):
    ...
    try:
        ...
        if include_zero:
            query = "SELECT CONVERT(user_name USING utf8) as Username, CONVERT(user_registration USING utf8) as Registration_date, user_editcount from user WHERE user_is_temp = false ORDER BY user_editcount desc"
        else:
            query = "SELECT CONVERT(user_name USING utf8) as Username, CONVERT(user_registration USING utf8) as Registration_date, user_editcount from user WHERE user_is_temp = false AND user_editcount > 0 ORDER BY user_editcount desc"
        cursor = cnx.cursor()
        cursor.execute(query)
        res = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])
        cursor.close()
